...
// two different characters (the relogin-leak scenario needs a BM and a TK).
type slotDB struct {
	*fakeDB
...
// TestAffectsDoNotLeakAcrossCharacters is the issue #21 regression: a buff on
// the character being left (here a persisted BM transform, affect Type 16, plus
// a live Divine deadline) must NOT survive the return to the selection screen —
// the entity is per-connection, and before the fix the next character picked on
// the same session showed (and re-persisted!) the previous character's buffs.
func TestAffectsDoNotLeakAcrossCharacters(t *testing.T) {
	db := &slotDB{
		...
		bySlot: map[int]world.CharacterState{
			0: {
				...
				Affects:   []world.Affect{{Type: 16, Value: 2, Level: 100, Time: 140}},
			},
			1: {
	...
	playAndLogout(0) // the BM: transform + Divine rehydrated, then saved on logout
	...
	// carries the transform and the Divine deadline.
	if len(beastSave.Affects) != 1 || beastSave.Affects[0].Type != 16 {
	...
	if len(knightSave.Affects) != 0 {
		t.Errorf("TK save affects = %+v, want none (BM transform leaked across characters)", knightSave.Affects)
	}
