...
> **Status da Fase 5: COMPLETO.** Todos os 58 handlers têm contrato. Dois handlers permanecem com
> **UNVERIFIED interno** por serem subsistemas extensos (a enumeração completa fica para sub-lotes
> dedicados, sinalizada no próprio contrato): **`_MSG_Quest`** (2753 linhas — catalogar todos os
> `npcMode`/etapas/recompensas) e **`_MSG_MessageWhisper`** (1710 linhas — enumerar todos os comandos
> `#pragma region /<cmd>`). As receitas/taxas exatas das variantes de combine ficam na Fase 4 §3.
